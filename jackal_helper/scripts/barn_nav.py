...
def scanCallback(data):
    ...
    if not len(message.angles):
        message.angles = np.array(
            [data.angle_min + i * data.angle_increment for i in range(message.minind, message.maxind)])
...
def ray_check(goal):
    ...
    while replan == True:
        if goal_dist > 0.8:
            ...
            for idx in range(int(max(scan_ind - check_rays, message.minind)),
            ...
            if np.sum(check_obs) != 0:
                ...
                if (scan_ind < message.minind or scan_ind > message.maxind):
                    ...
                    r_goal = [message.posx + r_dist * m.cos(message.angles[scan_ind] + message.theta),
                              message.posy + r_dist * m.sin(message.angles[scan_ind] + message.theta)]
